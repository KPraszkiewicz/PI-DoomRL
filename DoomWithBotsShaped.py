...
class DoomWithBotsShaped(DoomWithBots):
    ...
    Rewards are shaped according to the multipliers.
    """

    def __init__(self, game, frame_processor, frame_skip, n_bots, combinated_buttons=True, shaping=True):
        super().__init__(game, frame_processor, frame_skip, n_bots, combinated_buttons)
    ...
    def step(self, action, array=False):
        ...
        state, reward, done, done2, info = super().step(action)
        ...
        return state, shaped_reward, done, done2, info

    def reset(self, seed=None, options=None):
        self._print_state()
        
        state = super().reset(seed, options)
